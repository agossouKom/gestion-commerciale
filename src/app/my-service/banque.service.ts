...
const setArchive = URL + '/archiverBanque';
...
export class BanqueService {
  ...
  getAll() {
    return this.http.get<Banque[]>(allData);
  }

  getById(id: any) {
    console.log(getPerId);
    console.log(id);
    return this.http.get<Banque>(`${getPerId}/${id}`);
  }
  rechercher(query: any) {
    return this.http.get<Banque[]>(`${rechercher}=${query}`);
  }
  getActif() {
    return this.http.get<Banque[]>(actif);
  }

  create(params: Banque) {
  ...
  update(id: number, value: Banque): Observable<Object> {
