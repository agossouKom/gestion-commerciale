...
import { Arrondissement } from '../my-modele/arrondissement';
import { TokenStorageService } from '../shared/token-storage.service';



const vileUrl = environment.apiUrl + '/api/v1/villes/actifs';

const urlBase = environment.apiUrl;
const apiPrefix = '/api/v1';
const entity = '/arrondissement';
...
export class ArrondissementService {
  ...
  loadVille(): Observable<Ville[]> {
    return this.http.get<Ville[]>(vileUrl, requestOptions);
  }

  getAll() {
    return this.http.get<Arrondissement[]>(allData);
  }

  getById(id: any) {
    //console.log(getPerId);
   // console.log(id);
    return this.http.get<Arrondissement>(`${getPerId}/${id}`);
  }

  getActif() {
    return this.http.get<Arrondissement[]>(actif);
  }

  create(params: Arrondissement) {
  ...
  update(id: number, value: Arrondissement): Observable<Object> {
