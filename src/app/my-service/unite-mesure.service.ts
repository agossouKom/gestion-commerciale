...
import { UniteMesure } from '../my-modele/unite-mesure';
import { TokenStorageService } from '../shared/token-storage.service';


const urlBase = environment.apiUrl;
const apiPrefix = '/api/v1';
const entity = '/uniteMesures';
...
export class UniteMesureService {
  ...
  getAll() {
    return this.http.get<UniteMesure[]>(allData);
  }

  getById(id: any) {
    //console.log(getPerId);
   // console.log(id);
    return this.http.get<UniteMesure>(`${getPerId}/${id}`);
  }

  getActif() {
    return this.http.get<UniteMesure[]>(actif);
  }

  create(params: UniteMesure) {
  ...
  update(id: number, value: UniteMesure): Observable<Object> {
